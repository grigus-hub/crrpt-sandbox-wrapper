...
CORRUPTJPEG_API int jpeg_get_quant_table(
	...
	if (!h || !out_table || len < 64) return JPEG_API_ERR_INVALID_PARAM;
	if (channel < 0 || channel >= h->num_components) return JPEG_API_ERR_INVALID_PARAM;

	JQUANT_TBL* tbl = h->dinfo.quant_tbl_ptrs[channel];
	if (!tbl) return JPEG_API_ERR_LIBJPEG;

	for (int i = 0; i < 64; ++i)
		out_table[i] = tbl->quantval[i];
	return 64;
}

CORRUPTJPEG_API int jpeg_set_quant_table(
	JpegHandle* h, int channel, const uint16_t* in_table, size_t len) {
	if (!h || !in_table || len < 64) return JPEG_API_ERR_INVALID_PARAM;
	if (channel < 0 || channel >= h->num_components) return JPEG_API_ERR_INVALID_PARAM;
	...
	for (int i = 0; i < 64; ++i)
		tbl->quantval[i] = (UINT16)in_table[i];
	return 64;
}
